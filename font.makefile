...
# 手動修正済みグリフ画像の一覧
GLYPT_PATCHS=$(shell if [ -d FontSources/$(FontName)_patchGlyphs/ ] ; then find FontSources/$(FontName)_patchGlyphs/*.svg | xargs echo ; fi)


#  FontSources/$(FontName)_patchGlyphs/*.svg
$(FontFile): FontSources/$(FontName)_complete.svg FontSources/$(FontName)_list.txt $(GLYPT_PATCHS)
	# フォント画像の分割を実行
